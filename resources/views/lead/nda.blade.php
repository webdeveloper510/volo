            ...
            <div class="col-md-12">
                <form method="POST" action="{{route('lead.ndaresponse',urlencode(encrypt($lead->id)))}}" id='formdata'>
                    ...
                    <div class="row">
                        ...
                        <p>THIS AGREEMENT (the "Agreement”) is entered into on this <b>{{ base64_decode(urldecode(request()->query('day'))) }}</b> day of <b>{{ base64_decode(urldecode(request()->query('by'))) }}</b> by and
                        between VoloFleet, LLC & VoloFleet Logistics Holdings Corporation, (the” Disclosing Party”), and <b>{{ base64_decode(urldecode(request()->query('rec_p'))) }}</b> (the “Receiving Party”).

                        The Receiving Party hereto desires to participate in discussions regarding <b>{{ base64_decode(urldecode(request()->query('tran'))) }}</b> (the “Transaction”). During these discussions, Disclosing Party may share certain proprietary information with the Receiving Party. Therefore, in consideration of the mutual promises and covenants contained in this Agreement, and other good and valuable consideration, the receipt and sufficiency of which is hereby acknowledged, the parties hereto agree as follows:</p>
