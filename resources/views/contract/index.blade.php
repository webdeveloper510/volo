...
@section('action-btn')
<!-- @if (\Auth::user()->type == 'owner' || \Auth::user()->type == 'snr manager' || \Auth::user()->type == 'admin' && \Auth::user()->type != 'Accountant' && \Auth::user()->type != 'Manager') -->
@if($userRoleType == 'company')
@can('Create E-Sign')
<a href="{{ route('contracts.create') }}" class="btn btn-sm btn-primary btn-icon m-1" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Create New E-Sign') }}"><i class="ti ti-plus text-white"></i></a>
@endcan
@endif
<!-- @endif -->
@endsection
